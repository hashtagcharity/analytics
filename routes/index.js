...
router.get('/', function (req, res) {
    res.render('index', { title: '#charity Analytics' });
});
...
router.get('/statistics', function (req, res) {
    MongoClient.connect(config.mongoPath, function (err, db) {
        ...
        subs.aggregate([
            ...
            var dbResult = _.map(subResult, function (r) { return { x: new Date(r._id.year, r._id.month - 1, r._id.day), y: [r.count] }; });
            _.each(dates, function (date) {
                var r = _.filter(dbResult, function (dbRes) {
                    return dbRes.x.getTime() == date.getTime()
                });
                if (r.length == 0) {
                    dbResult.push({ x: date, y: [0] });
                }
            });

            var result = _.sortBy(dbResult, function (r) { return r.x; });
            _.each(result, function (r) {
                var earlier = _.filter(result, function (innerResult) {
                    return innerResult.x.getTime() <= r.x.getTime();
                });
                var sum = _.reduce(earlier, function (memo, value) { return memo + value.y[0]; }, 0);
                r.y.push(sum);
            });
